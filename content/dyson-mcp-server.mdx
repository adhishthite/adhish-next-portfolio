...
summary: "Built an MCP server so I can tell Claude 'make the air quieter' and have it actually happen. Teaching an LLM to speak MQTT was far more interesting than I expected."
...
I fixed it upstream, and the room temperature reads correctly now.
...
The agent reasons about intent, selects the appropriate tools, executes them, and synthesizes the results back into natural language, which keeps the whole thing both reliable and easy to extend.
