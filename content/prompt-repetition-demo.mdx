...
All you do is copy-paste your prompt twice. The paper tested this across 70 model-benchmark combinations and found 47 improvements and 0 regressions, with no case where doubling the prompt actually hurt accuracy.

I didn't believe it at first, so I built a demo to check.

---

## Why This Actually Makes Sense

The mechanism falls straight out of how causal language models work.
...
MiddleMatch at 2.8% baseline for Gemini Flash is essentially random. At 60.0% after repetition, it's working. The underlying capability was already there; what the model lacked was a view of the full context in both directions.
...
I work in LLM systems at Elastic, and a pattern I see constantly is people reaching for complex solutions to context problems: RAG, fine-tuning, chain-of-thought prompting. Some of those are the right tool. But a lot of the time the model already has the capability and simply can't see the full context, because of how attention works.

This paper is a clean example of that, and the technique barely costs you anything. The prompt gets longer (doubling it increases input tokens), but output length and measured latency are unchanged because the repetition happens in the parallelizable prefill stage. For longer prompts it's less viable. For the typical task prompt that's under a few hundred tokens, it's a free improvement.
