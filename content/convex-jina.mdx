---
title: "convex-jina: Accepted into the Official Convex Components Directory"
publishedAt: "2026-02-07"
summary: "convex-jina wraps Jina AI's Reader and Search APIs for Convex apps, with durable caching, reactive queries, and clean web content extraction. Now live in the official Convex Components directory."
pinned: false
---

# convex-jina: Accepted into the Official Convex Components Directory

> **Update (March 10, 2026):** convex-jina is now live in the official Convex Components directory: <a href="https://www.convex.dev/components/convex-jina" target="_blank" rel="noopener noreferrer" style={{color: "rgb(245, 135, 61)"}}>convex.dev/components/convex-jina</a>
...
The result: <a href="https://www.npmjs.com/package/convex-jina" target="_blank" rel="noopener noreferrer" style={{color: "rgb(245, 135, 61)"}}>convex-jina</a> - a Convex component that wraps Jina AI's Reader and Search APIs. Once installed, you pass it a URL and get clean markdown back, or pass a search query and get structured results optimized for LLMs.
...
It does two things, and does both of them well.

**Reader** - Give it any URL. It calls Jina AI's Reader API and returns clean markdown, HTML, or plain text, so you're not scraping HTML yourself or fighting with parsers. Jina handles the extraction; the component handles caching and state.
...
Both operations are durably cached in your Convex database with configurable TTL, so once you've read a URL the subsequent requests hit the cache. Usage tracking is built in too, which lets you see token consumption per user and per operation type.

## Why Jina AI?

I've tried a bunch of web extraction tools, and Jina's Reader API is genuinely good. It handles JavaScript-rendered pages, strips out navigation and ads, and the markdown it returns actually looks like the article content. Their Search API returns results with clean extracted content instead of just titles and URLs.

The free tier gives you 1 million tokens per month, which is plenty for most use cases. The API itself is dead simple: one endpoint, straightforward params, and output you can rely on.
...
The reactive part is key. Because this is Convex, your frontend automatically updates when the read operation completes, with no polling or WebSocket setup on your end. The component triggers an action, stores the result, and your query subscription picks it up.
...
The reasoning is that this is a *component* other developers install, and their users might each have their own Jina API key. Hardcoding it as a server-side env var would mean one key for the whole app, whereas passing it per-call lets each user bring their own.

The demo app stores it in localStorage: enter your key, it persists in your browser, and every API call includes it, with no server-side secrets needed.
...
Jina also has an Embeddings API. I considered wrapping it too, but Convex doesn't have native vector search. I could have stored embeddings and done brute-force cosine similarity, but that felt like a hack, and I'd rather only ship what works well.

Reader and Search is the right scope for this component. If Convex adds vector search support, embeddings would be a natural addition.
...
This is handled inside the Convex action, so the caller doesn't need to think about it and ends up with either a result or a clean error.
...
I entered this in the Third-Party Sync category, which is for components that sync data between Convex and external services. Reader syncs web content into your Convex database and Search syncs search results, both with caching and reactive queries.

Whether or not it wins, building and publishing a real npm package that other developers can install and use was worth it. Going from "I use this framework" to "I extended this framework" is its own kind of reward.
