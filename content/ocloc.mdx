...
Meet <a href="https://github.com/adhishthite/ocloc" target="\_blank" rel="noopener noreferrer" style={{color: "rgb(245, 135, 61)"}}>ocloc</a> (pronounced "oh-clock")—a lines-of-code counter that's stupidly fast. We're talking 2.3 million lines per second on my M2 MacBook. The entire Elasticsearch codebase (31,000 files, 5.5 million lines) analyzed in 2.35 seconds.
...
- **memchr** for finding newlines. This uses SIMD instructions under the hood—we're talking vectorized searching that processes multiple bytes in a single CPU instruction.
...
But the killer feature is **diff mode**—perfect for CI pipelines:
...
**elasticgpt-agents** (small repo)
- 302 files, 53K lines
- cloc: 0.45s → ocloc: 0.07s → **6.4x faster**

**elasticsearch** (large repo)
- 31K files, 5.5M lines
- cloc: 56s → ocloc: 2.35s → **23.8x faster**
