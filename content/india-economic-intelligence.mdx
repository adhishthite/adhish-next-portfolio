...
There's no API key, no registration step - you connect and query.
...
Each indicator gets its own interactive chart with zoom and hover. It's the same data economists and analysts use, pulled straight from the source with nothing in between.

The UI uses glassmorphism with dark mode and smooth loading skeletons. It looks nice, but the real value is that it pulls live government data on every visit, so there are no stale CSVs sitting around waiting for a manual refresh.

### How the MCP workflow works

MoSPI's MCP server follows a strict 4-step workflow:
...
This one is more involved. It visualizes data from the Periodic Labour Force Survey (PLFS), India's primary employment survey, covering 2017-18 through 2023-24 across all 36 states and union territories.
...
It wasn't a hard technical problem so much as the kind of dependency friction that quietly eats an afternoon.
...
India generates a massive amount of economic data, and most of it sits buried in PDFs and spreadsheets on government websites. Having MoSPI expose it through a machine-readable protocol changes what you can build on top of it.

These three projects are proof of that. You can put together real-time economic intelligence tools using nothing but open government data and open-source software - a free MCP endpoint and some TypeScript, with no proprietary data feeds or a Bloomberg terminal anywhere in the picture.

Everything is MIT licensed, so the code is there to clone and fork.
...
All three are open source, running on data your government already publishes.
